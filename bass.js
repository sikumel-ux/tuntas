...
// LANGSUNG JALANKAN ENGINE TANPA SYARAT APAPUN AGAR TOMBOL NAVIGASI BERFUNGSI INSTAN
initEventListeners();
cekSessionWarga();
...
function initEventListeners() {
    const ikat = (id, event, fn) => {
        ...
        if (el) {
            el.removeEventListener(event, fn); // Bersihkan sisa event lama jika ada
...
function cekSessionWarga() {
    ...
    if (savedKey && savedData) {
        ...
        document.getElementById('labelBulanBergabung').innerText = AKUN_WARGA_LOGGED_IN.bulan_bergabung || 'Mei 2026';
...
function switchWargaTab(id) {
    ...
    const targetBtn = document.getElementById(map[id]);
