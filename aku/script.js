...
// VERSION 3.0 - RESTRUCTURED ENGINE FOR UNIFIED GAS INTEGRATION (ANTI-CORS)
// ==========================================================================
...
function renderDataTabel() {
    ...
    dbGlobal.kas.forEach(trx => {
        ...
        if(tTrx >= tMulai && tTrx <= tSelesai) {
            ...
            cont.innerHTML += '<div class="flex justify-between items-center p-3 bg-white border border-slate-100 rounded-2xl animate-fade-in">' +
                '<div><p class="text-xs font-black uppercase text-slate-700">' + trx.Keterangan + '</p><p class="text-[9px] font-bold text-slate-400 mt-0.5">' + trx.Tanggal.split('T')[0] + '</p></div>' +
                ...
                '<button onclick="hapusTrx(\'kas\', \'' + trx.rowNum + '\')" class="text-slate-300 hover:text-red-500"><span class="material-symbols-rounded !text-sm">delete</span></button></div>' +
            '</div>';
        }
    });
    ...
    dbGlobal.anggota.forEach(w => {
        ...
        cWarga.innerHTML += '<div class="flex justify-between items-center p-3 bg-white border border-slate-100 rounded-2xl animate-fade-in">' +
            ...
            '</div>' +
            '<button onclick="hapusTrx(\'anggota\', \'' + w.rowNum + '\')" class="w-8 h-8 flex items-center justify-center text-slate-300 hover:text-red-500"><span class="material-symbols-rounded !text-md">delete</span></button>' +
        '</div>';
    });
...
// --- CORE REST POST ENGINE MURNI (ANTI-CORS BYPASS) ---
async function kirimKeServerTuntas(targetSheet, payloadObject, msgSukses) {
    showLoading();
    try {
        const response = await fetch(`${SCRIPT_URL}?action=insertData&sheet=${targetSheet}`, {
            method: "POST",
        ...
        if(hasil.status === "success") {
            tuntasAlert("Berhasil", msgSukses);
            reloadData();
        } else {
            throw new Error(hasil.message || "Gagal memproses data di spreadsheet.");
        }
    } catch (e) {
        hideLoading();
        console.error("Detail Error POST Engine:", e);
        tuntasAlert("Gagal", "Koneksi terputus dengan gerbang Google Sheets Backend: " + e.message, "error");
    }
}
...
function simpanKas() {
    ...
    const payload = {
        ...
        Kategori: kat,
        Keterangan: ket,
        Nominal: parseFloat(nom)
    };
    
    closeModal('mKas'); 
    kirimKeServerTuntas("kas", payload, "Data transaksi kas umum berhasil disimpan ke database!");
}
...
async function simpanAnggota() {
    ...
    try {
        ...
        if (fileInput && fileInput.files.length > 0) {
            ...
            const base64Kompresi = await kompresiFotoAdmin(file);
            
            // Simpan gambar terlebih dahulu ke Drive via action updateProfilWarga
            const uploadPayload = {
                image: base64Kompresi,
                filename: `AVATAR_${name.replace(/\s+/g, '_')}_${Date.now()}.jpg`,
                mimeType: "image/jpeg",
                folderId: "1FNIuGWMADgYO2Kk-KLmemSKmDzu46LM1"
            };
            
            const resUpload = await fetch(`${SCRIPT_URL}?action=updateProfilWarga&hp=${hp}`, {
                method: "POST",
                headers: { "Content-Type": "text/plain;charset=utf-8" },
                body: JSON.stringify(uploadPayload)
            });
            const hasilUpload = await resUpload.json();
            if (hasilUpload.status === "success") {
                base64Gambar = hasilUpload.url;
            }
        }
        
        // Buat objek payload warga baru untuk di-insert ke sheet anggota
        const payloadWarga = {
            Nama: name,
            Hp: hp,
            Password: '12345',
        ...
        kirimKeServerTuntas("anggota", payloadWarga, "Warga baru sukses didaftarkan dengan kata sandi bawaan: 12345");
    } catch (e) {
        hideLoading();
        tuntasAlert("Gagal", "Sistem gagal meregistrasikan warga: " + e.message, "error");
    }
}
...
function simpanIuran() {
    ...
    kirimKeServerTuntas("pembayaran", payload, "Catatan pembayaran iuran komunal berhasil dibukukan! No Ref: " + noRef);
...
function simpanLaporanSampah() {
    ...
    const payload = {
        Tanggal: tgl,
        Nama: nama,
        Status: radioStatus.value,
        Waktu: new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) + " WIB"
    };
    
    kirimKeServerTuntas("sampah", payload, "Laporan operasional angkutan sampah berhasil diverifikasi!");
}

// --- Hapus Data Terintegrasi Berdasarkan Baris Spreadsheet ---
function hapusTrx(namaSheet, rowNumber) {
    tuntasConfirm("Hapus data ini secara permanen dari baris database " + rowNumber + "?", async function() {
        showLoading();
        try {
            const res = await fetch(`${SCRIPT_URL}?action=deleteData&sheet=${namaSheet}&row=${rowNumber}`, {
                method: "POST"
            });
            const hasil = await res.json();
            hideLoading();
            if(hasil.status === "success") {
                tuntasAlert("Berhasil", "Data baris " + rowNumber + " berhasil dihapus dari sheet " + namaSheet);
                reloadData();
            } else {
            ...
            hideLoading();
            tuntasAlert("Gagal", "Sistem gagal menghapus data: " + e.message, "error");
        }
    });
}
    ...
    if(riwayat.length > 0) {
        riwayat.forEach(r => {
            list.innerHTML += '<div class="p-2.5 bg-slate-50 rounded-xl flex justify-between items-center text-[11px] animate-fade-in">' +
                '<div><p class="font-black text-emerald-900">' + r.Keterangan + '</p><p class="text-[9px] text-slate-400 font-bold">' + r.Tanggal.split('T')[0] + '</p></div>' +
                '<div class="flex items-center gap-2"><span class="font-extrabold text-slate-700">' + formatRupiah(r.Nominal) + '</span>' +
                '<button onclick="closeModal(\'mDetailIuran\\); hapusTrx(\'pembayaran\', \'' + r.rowNum + '\')" class="text-slate-300 hover:text-red-500"><span class="material-symbols-rounded !text-sm">delete</span></button></div>' +
            '</div>';
        });
    } else { 
