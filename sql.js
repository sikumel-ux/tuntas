...
// SOLUSI UTAMA: Jalankan langsung jika DOM sudah siap, jangan tunggu event jika telat
if (document.readyState === 'loading') {
    window.addEventListener('DOMContentLoaded', jalankanAplikasiPertamaKali);
} else {
    jalankanAplikasiPertamaKali();
}

function jalankanAplikasiPertamaKali() {
    initEventListeners();
    cekSessionWarga();
}

function initEventListeners() {
    // Gunakan pengecekan aman agar tidak crash jika salah satu elemen tidak sengaja hilang
    const formLogin = document.getElementById('formLoginWarga');
    if (formLogin) formLogin.addEventListener('submit', prosesLoginWarga);

    const formSaran = document.getElementById('formSaranWarga');
    if (formSaran) formSaran.addEventListener('submit', kirimSaranAspirasi);

    const formPass = document.getElementById('formPassWarga');
    if (formPass) formPass.addEventListener('submit', perbaruiPasswordWarga);
    
    const btnRef = document.getElementById('btnRefresh');
    if (btnRef) btnRef.addEventListener('click', sinkronUlangWarga);

    const btnOut = document.getElementById('btnLogout');
    if (btnOut) btnOut.addEventListener('click', logoutWarga);

    const btnPdf = document.getElementById('btnCetakPdf');
    if (btnPdf) btnPdf.addEventListener('click', unduhPdfKasWarga);

    const btnWa = document.getElementById('btnKonfirmasiWa');
    if (btnWa) btnWa.addEventListener('click', bukaKonfirmasiWa);
    
    const fMulai = document.getElementById('filterMulaiWarga');
    if (fMulai) fMulai.addEventListener('change', muatKasMasyarakat);

    const fSelesai = document.getElementById('filterSelesaiWarga');
    if (fSelesai) fSelesai.addEventListener('change', muatKasMasyarakat);

    const inpFoto = document.getElementById('inputFotoWarga');
    if (inpFoto) inpFoto.addEventListener('change', prosesUnggahFotoWarga);
...
function cekSessionWarga() {
    ...
    if (savedKey && savedData) {
        ...
        if(document.getElementById('scr-login')) document.getElementById('scr-login').style.display = 'none';
        ...
        if (AKUN_WARGA_LOGGED_IN.foto && AKUN_WARGA_LOGGED_IN.foto !== 'default.png') {
            if(document.getElementById('imgProfilWarga')) document.getElementById('imgProfilWarga').src = AKUN_WARGA_LOGGED_IN.foto;
        }
        ...
        if(document.getElementById('scr-login')) document.getElementById('scr-login').style.display = 'flex';
    }
}
...
async function prosesLoginWarga(e) {
    if (e) {
        e.preventDefault();
        e.stopPropagation(); // Stop submit liar bawaan browser
    }
    
    const loader = document.getElementById('loadingOverlay');
    if (loader) loader.style.display = 'flex';
    
    const hpEl = document.getElementById('loginHp');
...
// JALUR TOL: Daftarkan fungsi ke objek window global agar bisa dipanggil langsung dari HTML onsubmit
window.prosesLoginWargaDirect = prosesLoginWarga;
...
async function muatKasMasyarakat() {
    try {
        ...
        const fMulai = document.getElementById('filterMulaiWarga');
        const fSelesai = document.getElementById('filterSelesaiWarga');
        if(!fMulai || !fSelesai) return;

        let start = new Date(fMulai.value);
        let end = new Date(fSelesai.value);
...
function bukaDetailSampahKalender(tanggal, jam, status) {
    ...
    if(document.getElementById('dtlSampahTgl')) document.getElementById('dtlSampahTgl').innerText = `\${tglDuaDigit}-06-2026`;
    ...
    reader.onload = function (event) {
        ...
        img.onload = function () {
            ...
            fetch(`${DB_URL}/warga_rt04/\${KEY_WARGA_LOGGED_IN}/foto.json`, {
                ...
                if(document.getElementById('imgProfilWarga')) document.getElementById('imgProfilWarga').src = compressedBase64;
...
function perbaruiPasswordWarga(e) {
    ...
    fetch(`${DB_URL}/warga_rt04/\${KEY_WARGA_LOGGED_IN}/password.json`, {
        ...
        localStorage.setItem('warga_data', JSON.stringify(AKUN_WARGA_LOGGED_IN));
        if(document.getElementById('formPassWarga')) document.getElementById('formPassWarga').reset();
