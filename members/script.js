...
function cekSessionWarga() {
    ...
    if (savedKey && savedData) {
        ...
        // Atur Info Profil Screen (Bulan Bergabung disematkan di sini sekarang)
...
// FIX LOGIKA URUTAN: INPUT TERBARU DI DATABASE SEKARANG BERADA DI ATAS RENDERING UI
async function muatKasMasyarakat() {
    ...
    // 1. Kalkulasi saldo akumulatif maju (agar nilai hitungan keuangan tetap konsisten dari awal)
    keys.forEach(key => {
        const v = data[key];
    ...
    // 2. Balik urutan key array objek agar inputan paling gres/terbaru diproses duluan ke list UI
    ...
    reversedKeys.forEach(key => {
        ...
        if(tglItem >= start && tglItem <= end) {
            ...
            // Append berurutan ke bawah dari array yang sudah dibalik (Reversed)
