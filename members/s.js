...
function cekSessionWarga() {
    ...
    if (savedKey && savedData) {
        ...
        document.getElementById('labelBulanBergabung').innerText = "Bergabung: " + (AKUN_WARGA_LOGGED_IN.bulan_bergabung || 'Juni 2026');
...
// FIX LOGIKA URUTAN KAS: MENGGUNAKAN ARRAY.SORT() BERDASARKAN TANGGAL DESCENDING
async function muatKasMasyarakat() {
    ...
    // 1. Hitung Saldo Keseluruhan secara akumulatif kronologis asli
    Object.keys(data).forEach(key => {
        const v = data[key];
    ...
    // 2. Ubah data objek Firebase menjadi Array agar bisa dimanipulasi dengan sort
    let arrayKas = [];
    Object.keys(data).forEach(key => {
        arrayKas.push(data[key]);
    });

    // 3. URUTKAN BERDASARKAN TANGGAL (Terbaru di atas / Descending)
    arrayKas.sort((a, b) => new Date(b.tanggal) - new Date(a.tanggal));

    // 4. Filter berdasarkan rentang waktu pilihan warga lalu render ke HTML
    arrayKas.forEach(v => {
        const nom = parseInt(v.nominal) || 0;
        ...
        if(tglItem >= start && tglItem <= end) {
            ...
            // Simpan ke array filter global untuk cetak berkas PDF
            DATA_KAS_TERFILTER.push(v);

            // Tampilkan ke dalam daftar list UI mutasi kas
...
// FIX LOGIKA KALENDER: SEKARANG OTOMATIS DAN DINAMIS MENGIKUTI BULAN AKTIF BERJALAN
async function muatSampahSaya() {
    ...
    // Deteksi bulan & tahun hari ini secara real-time
    const hariIni = new Date();
    const tahunSekarang = hariIni.getFullYear();
    const bulanSekarang = hariIni.getMonth() + 1; 
    const stringBulanDuaDigit = String(bulanSekarang).padStart(2, '0');

    // Dapatkan total jumlah hari dalam bulan aktif saat ini
    const jumlahHariPenuh = new Date(tahunSekarang, bulanSekarang, 0).getDate();
    const mapSampahBulanIni = {};
    ...
    if (data) {
        Object.keys(data).forEach(key => {
            ...
            if (s.warga_key === KEY_WARGA_LOGGED_IN && s.tanggal) {
                ...
                // Cek kesesuaian tahun dan bulan transaksi
                if (parseInt(partTgl[0]) === tahunSekarang && partTgl[1] === stringBulanDuaDigit) {
                    const tanggalAngka = parseInt(partTgl[2]);
                    mapSampahBulanIni[tanggalAngka] = {
    ...
    // Render kotak kalender dinamis sesuai jumlah hari bulan saat ini
    for (let i = 1; i <= jumlahHariPenuh; i++) {
        let classWarna = "bg-slate-100 text-slate-700 hover:bg-slate-200";
        if (mapSampahBulanIni[i]) {
            const stat = mapSampahBulanIni[i].status.toLowerCase();
        ...
        const jamLog = mapSampahBulanIni[i] ? mapSampahBulanIni[i].jam : "-";
        const statusLog = mapSampahBulanIni[i] ? mapSampahBulanIni[i].status : "Belum Ada Log";

        boxKalender.insertAdjacentHTML('beforeend', `
            <div class="day-box ${classWarna}" onclick="bukaDetailSampahKalender('${i}', '${jamLog}', '${statusLog}', '${stringBulanDuaDigit}', '${tahunSekarang}')">
                ${i}
            </div>
        `);
    }
...
function bukaDetailSampahKalender(tanggal, jam, status, bulan, tahun) {
    const tglDuaDigit = String(tanggal).padStart(2, '0');
    document.getElementById('dtlSampahTgl').innerText = `${tglDuaDigit}-${bulan}-${tahun}`;
...
function kirimSaranAspirasi(e) {
    ...
    fetch(`${DB_URL}/saran_warga.json`, { method: 'POST', body: JSON.stringify(body) }).then(() => {
        input.value = "";
