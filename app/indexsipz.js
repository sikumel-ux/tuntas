...
async function unggahFotoProfil(input) {
    ...
    try {
        ...
        const fileNameStr = `AVATAR_${window.sessionWarga.Nama.replace(/\s+/g, '_')}_${Date.now()}.jpg`;

        // Bungkus data ke format JSON murni terstruktur (Bypass CORS Preflight via text/plain)
        const payload = { 
            image: base64Kompresi, 
            filename: fileNameStr, 
            mimeType: "image/jpeg",
            folderId: "1FNIuGWMADgYO2Kk-KLmemSKmDzu46LM1" // Drive Folder ID penampung file
        };

        // Menembak endpoint dengan parameter aksi updateProfilWarga
        ...
        if(hasil.status === "success") {
            ...
            // Optimistic UI Update: Langsung pasang URL foto baru dari respon server ke session local
            if (hasil.url || hasil.fileUrl) {
                window.sessionWarga.Foto = hasil.url || hasil.fileUrl;
                localStorage.setItem("tuntasWargaSession", JSON.stringify(window.sessionWarga));
            }
            
            // Tarik ulang database terbaru dari server spreadsheet
            muatDataWargaDariServer();
        } else {
        ...
        tuntasAlert("Gagal Unggah", "Sistem gagal memperbarui data foto Anda ke database.");
    } finally { 
        showLoading(false); 
        input.value = ""; 
    }
